...
class UserRoutes:
    ...
    def login_user_return_access_token(self, login, password):
        login_payload = {
            "login": login,
            "password": password,
        }
    ...
    def patch_user_data(self, access_token, login, password):
        headers = {
        ...
        response = requests.patch(f"{Constants.URL}/auth/user", data = data_payload, headers=headers)
        return response
